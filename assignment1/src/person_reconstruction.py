...
def estimate_poses_merge_at_end(N=1):  # 3.1
    ...
    for picture_no in tqdm(range(99, N - 1, -N)):
        ...
        _, R, t = icp.icp(source, target, method='kd', sampling='multires')
        R_list.append(R)
        t_list.append(t)

        source_reconstructed = source
        for i in range(len(R_list), 0):
            source_reconstructed = icp.apply_R_t_lists(source_reconstructed, R_list[i - 1], t_list[i - 1])
...
def estimate_poses_and_merge(N=4):  # 3.2
    ...
    for picture_no in tqdm(range(N, 99, N)):
        ...
        sampled_source, sampled_target = icp.uniform_sampling(merged_source, target, take_n=10000)
        _, R_list, t_list, _ = icp.icp(sampled_source, sampled_target, method='kd', sampling='none')

        source_reconstructed = icp.apply_R_t_lists(merged_source, R_list, t_list)
